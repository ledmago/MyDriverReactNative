    ...
    if (this.state.yolculukVarMi == false) {
      return (
        <View>
          ...
          <View>
            <View style={{ position: 'absolute', width: 100 + '%', left: 0, zIndex: 999999 }}>
            ...
            <Button disabled={this.state.step == 1} containerStyle={{ position: 'absolute', width: 70, left: 20, bottom: 120, zIndex: 999999999 }} title='Geri'

              onPress={() => {
                this.setState({ step: this.state.step - 1 })
              }}
            />
            <Button disabled={(this.state.kalkisCordinate.latitude < 1)} containerStyle={{ position: 'absolute', width: 70, right: 20, bottom: 120, zIndex: 999999999 }} title={this.state.step != 6 ? 'İleri' : 'Bitir'}
              onPress={async () => {
